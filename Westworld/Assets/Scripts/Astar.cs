...
public class PathFinder
{
    ...
    public PathFinder(SearchParameters searchParameters)
    {
        ...
        InitializeNodes(searchParameters.map);
        this.startNode = this.nodes[searchParameters.start.x, searchParameters.start.y];
        this.startNode.State = NodeState.Open;
        this.endNode = this.nodes[searchParameters.end.x, searchParameters.end.y];
    }
    private void InitializeNodes(bool[,] map)
    {
        this.width = map.GetLength(0);
        ...
        for (int x = 0; x < this.width; x++)
        {
            for (int y = 0; y < this.height; y++)
            {
                this.nodes[x, y] = new Node(x, y, map[x, y], this.searchParameters.end);
    ...
    public List<Point> FindPath()
    {
        ...
        if (success)
        {
            //Gizmos.DrawWireSphere(new Vector3(x,-5f,y),1.0f);
            Node node = this.endNode;
